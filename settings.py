...
class basic_setting():
    ...
    # 若交叉验证 验证集路径与训练集相同, 每次从中拆分出验证集
    val_data_root = '/home/sjh/dataset/LAG/image' if k_fold is None else data_root
    val_target_root = "/home/sjh/dataset/LAG/1200_Fovea_locations.csv" if k_fold is None else target_root

    crop_size = (256, 256)

    # #################################### train file settings ####################################
    run_dir = "REFUGE2_ROI_768"           # 备注数据集名称
    ...
    # #################################### test settings ####################################
    test_run_file = "2020-0917-1906_56_resnet34__fold_2"                     # 测试模型文件夹
    label_names = ["normal", "illness"]  # 类别名
    plot = True  # 保存测试预测图片
